...
const BomPDF: React.FC<BomPDFProps> = ({ bom, organization }) => {
  ...
  const logoPath = organization?.logo_path || null;
    ...
    >
      <Page size="A4" style={pdfStyles.page}>
        <View style={{ ...pdfStyles.tableRow, marginBottom: 20 }}>
          <View style={{ flex: 1, maxWidth: (logoPath ? 130 : 250) }}>
            {organization ? (
              ...
              {(item.alternatives?.length ?? 0) > 0 && (
                <React.Fragment>
                  ...
                  {item.alternatives?.map((alternative, i) => (
                    <View key={alternative.id || i} style={pdfStyles.tableRow}>
                      <Text style={{ ...pdfStyles.tableCell, flex: 0.05 }}></Text>
                      <View
                        style={{
                          ...pdfStyles.tableCell,
                      ...
                      <Text style={{ 
                        ...
                        {(alternative.quantity || 0)?.toLocaleString()} {item.measurement_unit?.symbol || ''}
                      </Text>
                    </View>
                  ))}
