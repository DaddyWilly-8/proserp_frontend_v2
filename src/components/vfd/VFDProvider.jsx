...
export function VFDProvider({ children }) {
  ...
  async function openPort(port) {
    ...
    try {
      // CRITICAL: Do NOT call port.open() if already open!
      ...
      if (!isAlreadyOpen) {
        ...
        console.log("VFD port already open – reusing connection");
      }

      // Always get fresh locks
      ...
      console.warn("Failed to open VFD port (will retry):", e.message);
      setConnected(false);
      // Don't cleanup here — let auto-reconnect try again
    }
  }
  ...
  useEffect(() => {
    ...
    navigator.serial.getPorts().then(async (ports) => {
      if (ports.length > 0) {
        console.log("Auto-connecting to existing VFD...");
        await openPort(ports[0]);
      }
    });
